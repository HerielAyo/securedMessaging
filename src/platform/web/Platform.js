...
function adaptUIOnVisualViewportResize(container) {
    ...
    const handler = () => {
        ...
        const offsetTop =
            sessionView.offsetTop +
            sessionView.offsetHeight -
...
export class Platform {
    constructor(
        ...
        if (assetPaths.serviceWorker && "serviceWorker" in navigator) {
            ...
            this._serviceWorkerHandler.registerAndStart(
                assetPaths.serviceWorker
            );
        }
        ...
        if (typeof fetch === "function") {
            this.request = createFetchRequest(
                this.clock.createTimeout,
                this._serviceWorkerHandler
            );
        } else {
    ...
    _createLogger(isDevelopment) {
        ...
        const transformer = (item) => {
            if (item.e?.stack) {
                item.e.stack = item.e.stack.replace(
                    /\/\?loginToken=(.+)/,
                    "?loginToken=<snip>"
                );
            }
        ...
        if (isDevelopment) {
            ...
            this.logger = new IDBLogger({
                name: "hydrogen_logs",
                platform: this,
                serializedTransformer: transformer,
            });
        }
    }
    ...
    saveFileAs(blobHandle, filename) {
        if (navigator.msSaveBlob) {
            ...
            downloadInIframe(
                this._container,
                this._assetPaths.downloadSandbox,
                blobHandle,
                filename,
                this.isIOS
            );
